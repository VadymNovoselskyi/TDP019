...
class Rule
  ...
  def parse
    @logger.debug("#{"[Rule:parse]".ljust(18)} Parsing rule '#{@name}' at pos #{@parser.pos}")

    @parser.rule_stack.push(@name)
    @logger.debug("#{"[Rule:parse]".ljust(18)} Entering parse for rule '#{@name}' at pos #{@parser.pos}")
    @logger.debug("#{"[Rule:parse]".ljust(18)} Current rule stack: #{@parser.rule_stack.inspect}")

    # Try non-left-recursive matches first, to avoid infinite recursion
    match_result = try_matches(@matches)
    @logger.debug("#{"[Rule:parse]".ljust(18)} Initial match result for rule '#{@name}' at pos #{@parser.pos}: #{match_result.inspect}")
    if match_result.nil?
      @parser.rule_stack.pop # Remove current rule from stack
      @logger.warn("#{"[Rule:parse]".ljust(18)} No matches found for rule '#{@name}' at pos #{@parser.pos}")
      return nil 
    end
    ...
    loop do
      ...
      if result.nil?
        ...
        @logger.debug("#{"[Rule:parse]".ljust(18)} Exiting parse for rule '#{@name}' at pos #{@parser.pos} with result: #{match_result.inspect}")
        ...
        @logger.info("#{"[Rule:parse]".ljust(18)} Rule '#{@name}' final result #{match_result.inspect}. Pos: #{@parser.pos}. Match stack: #{@parser.rule_stack.inspect}")
      ...
      @logger.debug("#{"[Rule:parse]".ljust(18)} Left-recursive match found for rule '#{@name}' at pos #{@parser.pos}, new result: #{result.inspect} overwriting previous result: #{match_result.inspect}")
      match_result = result
    end
  ...
  def try_matches(matches, pre_result = nil)
    ...
    matches.each do |match|
      @logger.debug("#{"[Rule:try_matches]".ljust(18)} Trying pattern #{match.pattern.inspect} for rule '#{@name}' at pos #{start}")
      ...
      match.pattern.each_with_index do |token,index|
        ...
        if @parser.rules[token]
          ...
          @logger.debug("#{"[Rule:try_matches]".ljust(18)} Matched '#{@name} = #{match.pattern[index..-1].inspect}'")
        else
          ...
          if nt
            ...
            @logger.debug("#{"[Rule:try_matches]".ljust(18)} Matched token '#{nt}' as part of rule '#{@name} <= #{pattern.inspect}'")
          else
            result = nil
            @logger.debug("#{"[Rule:try_matches]".ljust(18)} Pattern #{match.pattern.inspect} failed at pos #{@parser.pos}")
            @logger.debug("#{"[Rule:try_matches]".ljust(18)} Expected token #{token.inspect} but did not find it")
            break
          end
        end # pattern.each
      end # matches.each
      ...
      if result
        ...
        @logger.debug("#{"[Rule:try_matches]".ljust(18)} '#{@parser.tokens[start..@parser.pos-1]}' matched '#{@name}' and generated '#{match_result.inspect}'") unless match_result.nil?
...
class Parser
  ...
  def tokenize(string)
    ...
    until string.empty? # while there is still string left to tokenize
      @logger.debug("#{"[Parser:tokenize]".ljust(18)} Tokenizing remainder: #{string.inspect}")
      ...
      matched = @lex_tokens.any? do |tok|
        ...
        if match
          @logger.debug("#{"[Parser:tokenize]".ljust(18)} String #{match[0].inspect} consumed")
          # Also, evaluate this expression by using the block
          # associated with the token
          if tok.block
            result = tok.block.call(match.to_s)
            @logger.debug("#{"[Parser:tokenize]".ljust(18)} Token produced: #{result.inspect} of type #{result.class}")
            @tokens << result # Add last in token list
          else
            @logger.debug("#{"[Parser:tokenize]".ljust(18)} No token block for #{match[0].inspect}, skipping")
          end
      ...
      unless matched
        @logger.error("#{"[Parser:tokenize]".ljust(18)} Unable to lex '#{string}'")
  ...
  def parse(string)
    ...
    @logger.info("#{"[Parser:parse]".ljust(18)} Starting parse of string: #{string.inspect}")
    ...
    @logger.info("#{"[Parser:parse]".ljust(18)} Tokens after tokenize: #{@tokens.inspect}")
    ...
    begin
      ...
      @logger.error("#{"[Parser:parse]".ljust(18)} Parse failed at pos #{@pos}, max_pos #{@max_pos}: #{e.class}: #{e.message}")
      @logger.debug("#{"[Parser:parse]".ljust(18)} Expected tokens around failure: #{@expected.inspect}")
      raise
    end

    # If there are unparsed extra tokens, signal error
    if @pos != @tokens.size
      @logger.error("#{"[Parser:parse]".ljust(18)} Parse error at pos #{@pos}, max_pos #{@max_pos}: expected '#{@expected.join(', ')}', found '#{@tokens[@max_pos].inspect}' #{@tokens[@max_pos].class}")
      @logger.info("#{"[Parser:parse]".ljust(18)} All tokens were expected to be consumed, but #{@tokens.size - @pos} tokens remain unparsed. This indicates a syntax error in the input compared with the given rules.")
    ...
    if LoggerFactory.show_parse_trace?
      ...
      unless trace.empty?
        @logger.info("#{"[Parser:parse]".ljust(18)} PARSE TREE: All successfully matched rules in order - some risk for duplicates")
        @logger.info("-" * 50)
        trace.each do |entry|
  ...
  def expect(tok)
    ...
    t = next_token
    if @pos - 1 > @max_pos
      @logger.debug("#{"[Parser:expect]".ljust(18)} Updating max_pos to #{@pos - 1}, clearing expected tokens")
      @max_pos = @pos - 1
      @expected = []
    end
    @logger.debug("#{"[Parser:expect]".ljust(18)} Expecting #{tok.inspect}, got #{t.inspect} at pos #{@pos - 1}")
    return t if tok === t
    @logger.debug("#{"[Parser:expect]".ljust(18)} Token mismatch: expected #{tok.inspect} but found #{t.inspect}")
